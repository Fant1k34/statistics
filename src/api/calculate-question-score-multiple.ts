...
export const calculateQuestionScoreMultiple = (
    correct: Answer,
    current: Answer
) => {
    const defaultValues = {
    ...
    const correctArray = Object.entries(correct).reduce(
    ...
    const usersArray = Object.entries(current).reduce(
        (previousValue: string[], currentValue) =>
            currentValue[1]
                ? [...previousValue, currentValue[0]]
                : previousValue,
